...
class KMeans
{
    public:
        ...
        unsigned int k;
        bool no_data = false;
        KMeans(std::vector<cv::Point> &data_input):
            auto_k(true),
            data(data_input),
        ...
        KMeans(std::vector<cv::Point> &data_input, unsigned int k_input):
            k(k_input),
            auto_k(false),
            data(data_input),
            data_size(data_input.size())
        ...
        void clustering()
        {
            ...
            if(no_data == false)
            {
                ...
                for(k=k_start; k<=k_end; ++k)
                {
                    ...
                    for(int loop_times=0; loop_times<5; ++loop_times)
                    {
                        variance = 0;
...
class LaserHandler
{
    public:
	    ...
	    */
            ...
            float w_angle_min = -3.1415926*2.0/3.0;  // w in w_angle_min means wanted
            float w_angle_max = 3.1415926*2.0/3.0;
            float w_range_max = 1.5;
            ...
            line(mymap, map_origin, map_border_l, cv::Scalar(0, 0, 255));
            line(mymap, map_origin, map_border_r, cv::Scalar(0, 0, 255));
            // Draw points
            for(int i=0; i<data_nr_size; i++)
            {
                circle(mymap, data_nr[i], 0, cv::Scalar(0xAD, 0x53, 0x14), 1);
            }
            // Draw centers
        ...
        LaserHandler()
        {
            ...
            pub = n.advertise<car_msgs::LidarDetect>("cones", 10);
        }
