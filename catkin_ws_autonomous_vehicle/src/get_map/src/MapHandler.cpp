...
void MapHandler::visualizeSetPoints(int x,int y)
{
    // send them to rviz
    // ros::Rate r(10);
    ...
    // {
        if (x * y != 0)
        {
            ...
            p.pose.position.x = 3*x/100.0;
            p.pose.position.y = 3*y/100.0;
            p.pose.position.z = 0;
            p.pose.orientation.w = 1;
            p.pose.orientation.x = 0;
            p.pose.orientation.y = 0;
            p.pose.orientation.z = 0;
            p.scale.x = p.scale.y = p.scale.z = 0.04;
            p.color.g =p.color.a = p.color.r = 1.0;
            p.color.b = 0.0;
...
MapHandler::MapHandler(int map_h, int map_w) : map_height(map_h), map_width(map_w)
{
    ...
    // sub_cone = ros::Subscriber<car_msgs::LidarDetect>(nh, "/cones", 1);
    // sub_move = ros::Subscriber<car_msgs::car_location>(nh, "/location", 1);
    ...
    setpoint_pub = nh.advertise<visualization_msgs::Marker>("visualize/map", 1);
...
//     car.pos_x = car.start_x + (1.0 / ratio) * msg_move->x * 1;
//     car.pos_y = car.start_y + (1.0 / ratio) * msg_move->y * 1;
...
void MapHandler::callback_move(const car_msgs::car_location::ConstPtr &msg)
{
    ...
    // msg_temp = msg;
    // if positive direction of y-aixs is forward
    car.pos_x = car.start_x + (1.0 / ratio) * msg->x*1;
    car.pos_y = car.start_y + (1.0 / ratio) * msg->y*1;
    car.angle = car.start_angle + (-msg->z);
    // ROS_INFO_STREAM("x:"<<car.pos_x<<"|y:"<<car.pos_y);
    // draw line
    cv::line(map_track, cv::Point(car_past.pos_x, map_height-car_past.pos_y), cv::Point(car.pos_x, map_height-car.pos_y), cv::Scalar(255));
    // update car_past
    car_past.pos_x = car.pos_x;
...
void MapHandler::get_cone_zzx(const car_msgs::LidarDetect::ConstPtr &msg)
{
    ...
    int delta = 50;
    int kernel_size = 5; 
    int shift = (kernel_size%2!=0)?(kernel_size-1)/2:(kernel_size+1)/2;
    float cone_radius = 0.1;
    float k_e = 1;
    for (int i = 0; i < 8; ++i)
    {
        pix_x = 0;
        pix_y = 0;
        if (msg->x[i] != 0 && msg->y[i] != 0 && sqrt(msg->x[i]*msg->x[i]+msg->y[i]*msg->y[i])<1.5)
        {
            lidar_x_new = msg->x[i] * (1+cone_radius*exp(-k_e*sqrt(msg->x[i]*msg->x[i]+msg->y[i]*msg->y[i]))/sqrt(msg->x[i]*msg->x[i]+msg->y[i]*msg->y[i]));
            lidar_y_new = msg->y[i] * (1+cone_radius*exp(-k_e*sqrt(msg->x[i]*msg->x[i]+msg->y[i]*msg->y[i]))/sqrt(msg->x[i]*msg->x[i]+msg->y[i]*msg->y[i]));
            pix_x = (int)(car.pos_x + (1.0/ratio) * (lidar_x_new * cos(car.angle) + lidar_y_new * sin(car.angle)));
            pix_y = map_height - (int)(car.pos_y + (1.0/ratio) * (-lidar_x_new * sin(car.angle) + lidar_y_new * cos(car.angle)));
            // draw cones on the map
            visualizeSetPoints(pix_x-car.start_x,map_height-pix_y-car.start_y);
            if (pix_x >= shift && pix_x+shift < map_width && pix_y >= shift && pix_y+shift < map_height)
            {
                for(int kernel_row=0; kernel_row<kernel_size; kernel_row++)
                {
                    for(int kernel_column=0; kernel_column<kernel_size; kernel_column++)
                    {
                        map_meta_zzx[pix_y-shift+kernel_row][pix_x-shift+kernel_column][1] = \
                        (map_meta_zzx[pix_y-shift+kernel_row][pix_x-shift+kernel_column][1]+delta < 255) ? \
                        (map_meta_zzx[pix_y-shift+kernel_row][pix_x-shift+kernel_column][1] \
                        +round(delta/(1+(abs(-shift+kernel_row)>abs(-shift+kernel_column)?abs(-shift+kernel_row):abs(-shift+kernel_column))))) : 255;
                    }
                }
            }
        }
...
void MapHandler::save_map(void)
{
    std::string map_path;
    std::string map_name;
    if(map_cv_zzx.empty() == false)
    {
        bool got_path = ros::param::get("get_map/map_path", map_path);
        bool got_name = ros::param::get("get_map/map_name_zzx", map_name);
        //ROS_INFO_STREAM("11111"<<map_path<<map_name);
        if(got_path && got_name)
        {

            //ROS_INFO_STREAM("11112");
            std::stringstream ss;
            std::stringstream ss_track;
            ss << map_path << map_name;
            ss_track << map_path << "track.png";
